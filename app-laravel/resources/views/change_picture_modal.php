<div id="profile-form-modal" class="profile-form-modal">
    <form id="profile-form" class="profile-form" enctype="multipart/form-data">
        <span class="back-button"><img src="/img/arrow_back.svg" alt="Back" class="back-icon" id="close-profile-form-modal"></span>
        <h2>Zdjęcie profilowe</h2>
        <label for="file-input" class="file-input-label">Wybierz plik</label>
        <input type="file" id="file-input" class="file-input" name="profile_picture" accept="image/*">
        <p id="file-name" class="file-name"></p>
        <div class="form-buttons">
            <button type="submit" id="upload-button">Zmień zdjęcie</button>
        </div>
    </form>
</div>
